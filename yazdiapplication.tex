...
%                      &\vdots \\
 ...
 % relative orientability by Proposition \ref{prop:relative-orientability}.
  ...
  We now need to show that $F_{n,k}$ is relatively orientable. We construct a outwards pointing normal vector field by combining the outwards pointing vector fields on $\widehat{F}$ and $A$ given by following $\gamma$ along the suspension flow.
  Consider a small neighbourhood $U$ of $\gamma$: define two bump functions $c_1$ and $c_2$ supported in $U$ such that $c_1$ is $1$ on $A$, and decays to $0$ in $\widehat{F}$ and $c_2$ is $1$ on $\widehat{F}$ and decays to $0$ on $A$.
  ...
  Observe that since $v_1$ points in the flow direction, and $v_2$ points into the surface, the new vector field $c_1 v_1 + c_2 v_2$ is transverse, in the neighbourhood of $\gamma$, to $F_{n,k}$ (see \autoref{fig:gluing-normal-field} for a picture of the resulting transverse vector field).

  We perform a similar construction in a small neighbourhood of $\widehat{\gamma}$: in this case, the fact that the vector field on $\widehat{F}$ points in the flow direction, and the vector field on $A$ points away from the surface $\widehat{F}$ ensures that the new vector field is transverse, in a neighbourhood of $\widehat{\gamma}$, to the surface $F_{n,k}$.

  A key fact we use in this construction is that the vector field along $A$ that starts pointing into $\widehat{F}$ at $\gamma$ comes back pointing away from the surface at $\widehat{\gamma}$: this is because the homeomorphism $f_{n,kj}$ maps the inner tubular neighbourhood of $\gamma$ to the outer tubular neighbourhood of $\widehat{\gamma}$, where inner and outer tubular neighbourhood are the half tubular neighbourhoods contained in $\widehat{F}$ and the complement respectively.
...
\begin{figure}[t]
  ...
  \caption{Combining the vector fields over $\widehat{F}$ and $A$ by adding them after scaling with appropriate bump function. The right hand side picture shows that the resulting vector field is transverse to the surface $F_{n,k}$.}
