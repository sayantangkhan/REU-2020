...
negative Dehn twist. As we saw in Section \ref{sec:backgr-mapp-class}, in order to perform the Penner
...
\begin{proof}
  ...
  through even in this setting %essentially
  because of the local nature of the proof. Moreover, its Poincar\'e dual is in $\overline{\mathcal{K}_{n,k}}$ by Theorem \ref{thm:classifying-fibrations}.
  \caleb[inline]{Do we need to say more here?}
...
\ref{sec:backgr-mapp-class} we saw that pseudo-Anosov maps give rise to matrices whose Perron-Frobenius eigenvalue is
...
\item There exists a constant $D'$, independent of $n$ and
  ...
  \ref{sec:backgr-mapp-class} that we refer to the linear action of $f_{n,k}$ on the subspace of the cone of
...
\begin{proof}
  ...
  the function described in Theorem \ref{thm:fm}. Note that we have the following bounds on $g_{n,k}^r$.
  \begin{align*}
    g^r_{n,k} &= g_{n,k} + r \\
              &\leq g_{n,k} + 14n \\
              &< 2g_{n,k}
