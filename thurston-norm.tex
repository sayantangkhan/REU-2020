...
A mapping torus is a surface bundle over $S^1$, or \emph{fibrations over $S^1$}, denoted $S\rightarrow M_\varphi\rightarrow S^1$.
...
\p{Complexity of an embedded surface} Let $M$ be an compact orientable closed $3$-manifold.
...
\p{Thurston norm} Let $a$ be a homology class in $H_2(M; \ZZ)$.  Define the integer valued norm $x:H_2(M;\ZZ)\rightarrow \ZZ$ as the following:
\begin{align*}
...
%There is a unique continuous extension of $x$ to $\RR$ which is a $\RR$-valued function on $H_2(M; \RR)$ called the \emph{Thurston norm}.
The unit ball for the Thurston norm is a convex polyhedron in $H_2(M;\RR)$.%, and thus it makes sense to talk about the \emph{faces} of the unit ball.
...
The open faces of the unit ball whose cones contain the fibers of fibrations are called the \emph{fibered faces}.
%The takeaway from this theorem is to determine if a surface $S$ embedded in a $3$-manifold is homologous to the fiber of some fibration, it suffices to check whether the homology class $[S]$ lies in one of the open cones corresponding to the fibrations.
...
%\p{Non-orientable surfaces} A non-orientable surface can be obtained from an orientable surface $S$ by attaching {\it crosscaps}.  
%%We will visualize non-orientable surfaces by thinking of them as orientable surfaces with \emph{crosscaps} attached.
%We attach a crosscap to $S$ by removing an open disk $D\subset S$, and identifying the boundary of that disc (on the surface) via the antipodal map.
%In figures we depict a crosscap by a circle with an inscribed X.  For example, \autoref{fig:buildingblock} depicts an oriented surface of genus 5 and three boundary components with two crosscaps attached.

%Let $\no_{g,n}$ be the non-orientable surface obtained by attaching $g$ crosscaps to $S^2$ and marking $n$ points in $S^2$.
...
%Let $\no$ be a non-orientable surface of genus $g$.  The orientation double cover of $\no$ is a pair consisting of an orientable surface $\wt{\no}$ of genus $g-1$ (as defined for orientable surfaces) and a covering map $p:\wt{\no}\rightarrow\no$ of degree 2.  The deck group of the covering map is thus isomorphic to $\mathbb{Z}/2\mathbb{Z}$ and consists of the identity and an orientation reversing homeomorphism $\iota$.



\p{Poincar\'e Duality}
To see why Poincar\'e duality for non-orientable 3-manifolds fails, we first consider the orientable case.  Let $M$ be an orientable manifold.  To define the Poincare dual of $H^1(M;\ZZ)$, we first define a homotopy class of maps $M\rightarrow S^1$.  Then we construct an element of $H^2(M;\ZZ)$.
...
%Changing the basepoint or the representative $1$-form gives a different map to $S^1$ that is homotopic to the original map 
(see Section 5.1 of \cite{calegari2007foliations} for the details).
%One can recover the $1$-form $\alpha$ from the map $f_{\alpha}$ by pulling back the canonical length form on $S^1$ along $f_{\alpha}$.

Now let $\theta \in S^1$ be a regular value so that $S = f^{-1}(\theta)$ is a surface.
...
\p{Defining the Thurston norm on cohomology} In order to define the Thurston norm on $H^1(N;\ZZ)$, we first need to relate $H^1(N; \RR)$ and $H^1(\wt{N}; \RR)$.
...
\begin{proof}
  ...
  Then there exists a smooth function $g:\wt{N} \to \RR$ such that the following relation holds:
    ...
    Since the cohomology class $[\alpha]$ is $\iota^\ast$-invariant, $\alpha$ and $\iota^{\ast}(\alpha)$ must differ by an exact form.
    \begin{align*}
    ...
    %we can push it forward by the projection map $p$, defining the value of its pushforward locally.
...
Lemma \ref{lem:injective} tells us that $H^1(N; \RR)$ is a subspace of $H^1(\wt{N}; \RR)$, so we define the Thurston norm on $H^1(N; \RR)$ by restricting the Thurston norm to the subspace $p^*(H^1(N;\RR))$ of $H^1(\widetilde{N};\RR)$.
...
We next see that the unit ball for the Thurston norm on $H^1(N;\RR)$ can be defined analogously to the orientable case.
\begin{thm}
  The unit ball with respect to the dual Thurston norm on $\left( H^1(N; \RR) \right)^{\ast}$ is a polyhedron whose vertices are lattice points $\{\pm \beta_1, \ldots \pm \beta_k\}$.
...
\begin{proof}
  ...
  The rest of the proof is identical to the orientable case.
\end{proof}
...
In Section \ref{sec:weak-inverse-poinc}, we develop a version of Poincar\'e duality for non-orientable 3-manifolds that will let us translate a family of embedded surfaces into $1$-forms, giving us a more concrete relationship between the Thurston norm and embedded surfaces for non-orientable 3-manifolds.

\subsection{Weak inverse to the Poincar\'e duality map}
\label{sec:weak-inverse-poinc}

Let $M$ be a 3-manifold.  Regardless of whether $M$ is orientable or not, we can construct a dual map $f_\alpha:M\rightarrow S^1$, using equation \eqref{form:map}.
...
In this section, we state a weaker version of Poincar\'e duality for non-orientable surfaces in Theorem \ref{thm:strong-duality}, that lets us associate $1$-forms to a certain class of embedded surfaces in non-orientable $3$-manifolds.
...
\end{prop}
\begin{proof}
  Let $\wt{N}$ be the orientation double covering space of $N$, and $\wt{\no}$ be the preimage of $\no$ under the covering map.
...
\begin{thm}[Poincar\'e Duality for non-orientable 3-manifolds]
  ...
  Then there exists $[\alpha] \in H^1(N; \ZZ)$ such that the pullback of $[\alpha]$ to $\wt{N}$ is dual to $\wt{\no}$ in $\wt{N}$.
...
\begin{lem}
  \label{lem:PD1}
  Let $N$ be a non-orientable 3-manifold and let $\wt{N}$ and the map $p:\wt{N}\to N$ be its orientation double cover. %Let $\iota$ be the non-trivial deck transformation.
...
\begin{proof}
  ...
  Since $\iota$ reverses the orientation on $\wt{N}$ but preserves the direction of $\iota(v_3)$, $\iota$ must reverse the orientation of the pair $(v_1, v_2)$.
  ...
  Then there exists a representative 1-form $\wt{\alpha}$ that is $\iota^{\ast}$-invariant.
...
Let $N$ be a non-orientable 2-manifold.  Let $[\wt{\alpha}]\in H^1(\wt{N}, Z)$ and let $\wt{N}$ be its Poincar\'e dual.  There exists $[\alpha] \in H^1(N; \ZZ)$ such that $\wt{\alpha} = p^{\ast} \alpha$.
\end{lem}
\begin{proof}
 % To show that $[\wt{\alpha}]$ is the pullback of an element in $H^1(N; \ZZ)$, let $\gamma$ be a curve it $N$.  
 It will suffice to show that for any simple closed curve $\gamma$ in $N$, the integral of $\wt{\alpha}$ along any path lift of $\gamma$ is an integer.  Let $x_0\in N$ be a base point of $\gamma$.  Note that $\gamma$ has two (path) lifts $\wt{\gamma}_1,\wt{\gamma}_2$ under $p$ in $\wt{N}$.  Either $\wt{\gamma}_1,\wt{\gamma}_2$ are both simple closed curves based at the each of the two preimages $p^{-1}(x_0)$ or $\wt{\gamma}_1,\wt{\gamma}_2$ are both arcs between the two points of $p^{-1}(x_0)$.
 ...
 % The next goal is to show that
  ...
  %\end{align*}
  Without loss of generality, we can assume that the curve that all intersections of $\wt{\gamma}$ with the surface $\wt{\no}$ are transverse.
...
\begin{lem}
  ...
  Let $N$ be a non-orientable 3-manifold and let $\wt{N}$ be the orientation covering space of $N$ with covering map $p$.  If $\no$ is a relatively oriented incompressible surface in $N$, then $p^{-1}(\no)$ is also incompressible.
\end{lem}
\begin{proof}
...
%  \end{align*}
Since $p_\ast:\pi_1(\wt{N})\rightarrow\pi_1(N)$ is injective, the induced map $\pi_1(\wt{\no}) \to \pi_1(\wt{N})$ must also be injective.  An injective induced map on fundamental groups is equivalent to the orientable surface $N$ being incompressible.
\end{proof}
...
\begin{proof}[Proof of Theorem \ref{thm:strong-duality}]
  ...
  By Lemma \ref{lem:PD2}, there exists a 1-form $\alpha\in H^1(N;\ZZ)$ such that $\wt{\alpha}=p^\ast\alpha$.  %Since $\wt{\alpha}$ is $\iota^\ast$ invariant by \ref{lem:PD1} and $\no$.

By equation (\ref{form:map}), we define the map $f_\alpha$.  Because $\alpha$ is non-vanishing, the map $f_{\alpha}: N \to S^1$ has full rank everywhere.  Therefore $f_\alpha$ is a fibration.
The map $f_{\alpha} \circ p$ is the lift of $f_{\alpha}$ to $\wt{N}$ under $p$, and is therefore also a fibration.
...
% \end{proof}
  ...
  To have that, we would require the isotopy on the orientation double cover to be $\iota^{\ast}$-invariant, but the theorem is sufficient for our application.
...
We close the section with a non-orientable version of Theorem \ref{thm:Thur1}.
\begin{thm}
...
\end{thm}
\begin{proof}
Let $\wt{N}$ be the orientation double cover of $N$ with covering map $p:\wt{N}\rightarrow N$.


For (i), we observe that by Theorem~\ref{thm:Thur1} the fibrations of $\wt{N}$ are in one-to-one correspondence with lattice points inside a union of cones over open faces of the unit ball in $H_2(\wt{N};\RR)$.  Let $\wt{\mathcal{K}}$ be the union of cones in $H^1(\wt{N};\RR)$.
 By Poincar\'e duality, $\wt{\mathcal{K}}$ maps to a union of cones in $H^1(\wt{N};\RR)$, which we will call $\wt{K}^\ast$
 
  The union of cones $\wt{K}^\ast$ in $H^1(N; \RR)$ is determined by intersecting the pullback of $H^1(N; \RR)$ to $H^1(\wt{N}; \RR)$ with $\wt{\mathcal{K}}$.
  Every lattice point in $\wt{K}^\ast$ corresponds to a fibration $f:N\to S^1$, since the pullback of $f$ to $H^1(\wt{N}; \ZZ)$ corresponds to a fibration of $\wt{N}$.
  Conversely, every fibration of $f:N\rightarrow S^1$ must lie in $\wt{K}^\ast$, since the composition $f\circ p$ is a fibration of $\wt{N}\rightarrow S^1$.
