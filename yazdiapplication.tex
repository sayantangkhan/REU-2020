...
\begin{proof}
    ...
    \caleb[inline]{I believe Thurston-norm minimizing for $\wt{F_{n,k}}$ is also the same, but why is it in Yazdi originally that surface homologous to $F$ but with smaller Thurston norm would have to contain an essential torus or sphere?}
\end{proof}

\caleb[inline]{Unless I'm missing something, I don't believe we need Lemma 4.6 anymore. From lemma 2.8 combined with the fact that something is in the 1 eigenspace of the action of the deck transformation on cohomology if and only if its Poincar\'e dual is in the -1 eigenspace of the action on homology, we know that the homology class of a surface upstairs that is a lift of a surface downstairs must be in the -1 eigenspace of the action on homology.}
\begin{lem}
...
\begin{proof}
    ...
    The homology class $[\wt{P^r_{n,k}}] = [\wt{P_{n,k}}] + r[\wt{F_{n,k}}]$ is clearly integral, and it is also primitive since there is a curve in $\wt{M_{n,k}}$ that intersects $\wt{P_{n,k}}$ transversely and exactly once, while avoiding $\wt{F_{n,k}}$. (?) \caleb{Maybe I'm getting myself turned around, but why does this have to be primitive? Are SK: I believe that's because the Fried-Matsumoto result requires primitive classes.}. Since $[P^r_{n,k}]$ is integral, primitive and lies in the fibered face $C_{n,k}$, by Theorem \ref{thm:Thur1} it is the fiber of a fibration of $\wt{M_{n,k}}$. Since the monodromy $\wt{f_{n,k}}$ is pseudo-Anosov, all monodromies that correspond to this face are pseudo-Anosov by Theorem \ref{thm:ThurHyp}, so in particular $\wt{f^r_{n,k}}$ is pseudo-Anosov.
