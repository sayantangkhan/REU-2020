...
We now need to show that $F_{n,k}$ is relatively orientable. We construct a outwards pointing normal vector field by gluing together the outwards pointing vector fields on $\widehat{F}$ and $A$ given by following $\gamma$ along the suspension flow.
In general, one cannot glue together such vector fields without making it vanish somewhere, but we control how $\widehat{F}$ and $A$ intersect: namely at $\gamma$ and $\widehat{\gamma}$.
Choose the outward direction over $\widehat{F}$ to be the forward flow direction.
Construct the outward direction over $A$ by constructing an outward vector field over $\gamma$: we define this to be the vector field that points into the surface $\widehat{F}$.
We then pushforward this vector field in the flow direction, the vector field over $\widehat{\gamma}$ points away from the surface $\widehat{F}$.
Indeed, the homeomorphism $f_{n,k}$ maps the intersection of the tubular neighbourhood of $\gamma$ and $\widehat{F}$ to the intersection of the tubular neighbourhood of $\widehat{\gamma}$ and the complement of $\widehat{F}$.
Therefore one can glue together the two non-vanishing vector fields over $\widehat{F}$ and $A$ to a non-vanishing vector field over the surface $F_{n,k}$, proving the relative orientability of this surface. See \autoref{fig:gluing-normal-field} for a picture of this gluing construction.
\begin{figure}[t]
  \centering
  \incfig[0.5]{gluing-normal-field}
  \caption{Gluing the normal vector fields over $\widehat{F}$ and $A$. The left to right direction is the flow direction, and the up to down direction is the fiber direction. The yellow region is $\widehat{F}$ in the fiber and blue is the complement of $\widehat{F}$ in the fiber.}
