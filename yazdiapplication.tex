...
\becca[inline]{Do we need to square? Is it true that the orientation double cover is a characteristic cover?  And if we do need to square, shouldn't the stretch factor square under powers?}
\sayantan[inline]{We definitely do need to square, depending on the mapping class we're lifting, and the squaring does square the stretch factor as well, but since our asymptotic result is about the log of the stretch factor, we incur a mulitplicative penality of $2$, which can be absorbed into the ambient constant.
Also, it is true that the subgroup of $\pi_1(\no_g)$ corresponding to $\pi_1(\os_{g-1})$ is left invariant by every group automorphism of $\pi_1(\no_g)$ induced by a homeomorphism of $\no_g$. That's because $\pi_1(\os_{g-1})$ can be generated by taking all two-sided curves in $\no_g$, which get mapped to two-sided curves by any homeomorphism.}
...
below as well. The more challenging part of the proof is showing the upper bound holds. %We will constructing pseudo-Anosov maps with small stretch factors, adapting Yazdi's techniques to the non-orientable setting.

We will closely follow Yazdi's construction, which proceeds in five steps.  In steps 1 and 2, he constructs a family of psuedo-Anosov homeomorphisms
...
boundary components $c,d$ and $e$. Choose an orientation for $S$ and let $c,d$ and $e$ inherit the induced orientations. We obtain a non-orientable surface $T$ from $S$ as follows.  Add two cross caps to $S$ (but retain the orientation of the boundary components of $S$). Remove a point $p$ from the boundary component $e$ and let $q$ be a marked point in $e$.\becca[inline]{From what's below, we actually puncture later, not in the initial construction}  Let  $r$ and
...
\becca[inline]{I commented out the discussion of why we use $T$ as the building block because it felt out of place (we just defined $T_{n,k}$, aren't we really treating that like the building block?) and because I didn't really understand the second point.  We define pseudo-Anosov of what?  The mapping torus we construct is not a mapping torus of $T$, it's a mapping torus of $T_{n,k}$}
...
\item We will construct a pseudo-Anosov homemorphism $\varphi$ of $T$\becca[inline]{this previously referenced ``a given pseudo-Anosov."  We mean of $T$, right?} such that $\gamma$ and $\varphi(\gamma)$ we construct form the boundary of an embedded
