...
\p{Mapping tori} Let $S$ be a surface, and let $\varphi: S \to S$ be a homeomorphism.  A {\it mapping torus} of $S$ by $\varphi$ is the $3$-manifold $M_\varphi$ given by the identification:
\begin{align*}
...
\p{Complexity of an embedded surface} Let $M$ be a compact orientable closed $3$-manifold.
...
%\becca[inline]{This part of the construction works for non-orientable manifolds, right?  I therefore took out the requirement that $M$ is orientable in the setup.}
%\sayantan[inline]{Yes, that is right.}
...
%   \becca[inline]{We were using $\theta$ both as a regular value and as a dummy variable.  I think I correctly changed some $\theta$s to $\xi$s (even though I hate $\xi$).}
...
%Lemma \ref{lem:injective} tells us that $H^1(N; \RR)$ is a subspace of $H^1(\wt{N}; \RR)$, so 
Next we use Lemma \ref{lem:injective} to define the Thurston norm on $H^1(N; \RR)$.% by restricting the Thurston norm to the subspace $p^*(H^1(N;\RR))$ of $H^1(\widetilde{N};\RR)$.
...
% Commenting out the theorem because it is not really used anywhere else.
%\begin{thm}
...
In Section \ref{sec:weak-inverse-poinc}, we develop a (weak) version of Poincar\'e duality for non-orientable 3-manifolds.% that will allow us to translate between
%\becca[inline]{This implies isomorphism.  Is there a more accurate way to say this?}
%\sayantan[inline]{One way to describe this would be an isomorphism between fibers of fibrations, and lattice points in a certain cone in $H^1(M, \mathbb{Z})$, where the isomorphism maps addition in the cone to the operation of oriented sums of fibers.}
%\becca[inline]{That's very similar to what we say in the next paragraph.  Perhaps we should just leave it out.}
%a family of embedded surfaces and certain $1$-forms, giving us a more concrete relationship between the Thurston norm and embedded surfaces for non-orientable 3-manifolds.
...
\p{Orientable manifolds} Now let $M$ be an orientable 3-manifold, and let $S$ be an orientable embedded surface.  If $S$ is relatively oriented with respect to $M$, then a choice of orientation on $S$ determines an orientation on $M$ and vice versa.

%\p{An orientable surface that is not relatively oriented in a 3-manifold}
%  Let $S$ be the standard torus $\RR^2/\ZZ^2$, and let $\varphi$ map $(x,y)$ to $(-x, y)$.
 ...
 % Let $\gamma$ be the line $x=0$ in $S$. %Then $\gamma$ is preserved by $\varphi$.
...
% Commented out because it is not necessary, and the proof/statement has gaps.
%\begin{prop}
...
\begin{proof}
  ...
  Next we show that $\iota$ reverses the orientation of $\wt{\no}$.  To do so, we first observe that because $\no$ is relatively oriented in $N$, the outward pointing transverse vector field on ${\no}$ must lift to an outward pointing transverse vector field on $\wt{\no}$.  In particular, for any outward pointing vector $\wt{v}$ on $\no$, the vector $\iota(\wt{v})$ is also outward pointing.%Therefore $\iota$ preserves outward pointing vector fields on $\wt{\no}$.\becca[inline]{This statement doesn't follow.  What is true is that $iota$ preserves outward pointing vector fields that are lifts of vector fields of $\no$.  This is all we need, but its awkward to say.}
  ...
  $\iota^{\ast}$-invariant.
%  \becca[inline]{Does this chain of inequalities prove that any representative is $\iota^\ast$-invariant?}
\end{proof}
...
For (i), we observe that by Theorem~\ref{thm:Thur1} the fibrations of $\wt{N}$ are in one-to-one correspondence with lattice points inside a union of cones over open faces of the unit ball in $H_2(\wt{N};\RR)$.  Let $\wt{\mathcal{K}}$ be the union of cones in $H_2(\wt{N};\RR)$.
 ...
 % Since the $\wt{\alpha}$ is the pullback of $\alpha$, the 1-form $\alpha$ lies in $K$ and  contains the 1-form corresponding to $f$.
\end{proof}
...
\p{Oriented sum for non-orientable manifolds} Let $N$ be a non-orientable 3-manifold and let $\no$ and $\no'$ be embedded surfaces in $N$ that are relatively oriented.
