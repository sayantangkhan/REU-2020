...
He showed the $l_{g,0}$ is bounded $$\frac{A}{g}\leq l_{g,0}\leq \frac{B}{g},$$ where $A,B$ are real constants.  Therefore $l_{g,0}$ asymptoically behaves like $\frac{1}{g}$.  Following Penner, many authors have found contraints on $l_{g,n}$ for $n\geq 0$.  In particular, Tsai found that when $S_{g,n}$ is orientable surface of fixed genus $g\geq 2$, $l_{g,n}$ is on the order of $\frac{\log(n)}{n}$ \cite{tsai2009asymptotic}. Much of this work has focused on orientable surfaces.  Let $\mathcal{N}_{g,n}$ be a non-orientable surface of genus $g$ with $n$ punctures.  In this paper we study the asympototic behavior of $l_{g,n}$ as $n$ increases.
