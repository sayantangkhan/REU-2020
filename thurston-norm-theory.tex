...
\begin{example}[Failure of surjectivity]
  ...
  of $H^1(\wt{M}; \ZZ)$ by simply assigning integer values to the basis elements. \caleb{Is this not an
    element of $H^1(\wt{M}, \RR)$? SK: Yes, it is, but $\alpha + \iota^{\ast}\alpha$ becomes an integer $1$-form.} Pick an element $\alpha$ that assigns $0.5$ to $\wt{\gamma}$ and an
...
We then perform a local surgery such that each $S$ leaf joins an $S'$ leaf. We have two possible choices: we
...
The normal vector field tells us that the left $\wt{S}$ leaf gets glued to the bottom $\wt{S'}$ leaf near
...
It turns out that the ``right'' way to think about fibrations $f: M \to S^1$ is by studying a specific kind of
